...
function getParallelepipedDiagonal(a, b, c) {
  const diagonalLenght = Math.sqrt(a ** 2 + b ** 2 + c ** 2);
...
function roundToPowerOfTen(num, pow) {
  const multi = 10 ** pow; // Math.pow(10, pow);
  return Math.round(Math.floor(num) / multi) * multi;
}
...
function getCountOfOddNumbers(number) {
  ...
  let count = 0;
  for (let i = 0; i <= numberPositive; i += 1) {
    if (i % 2 !== 0) {
      count += 1;
    }
  }
